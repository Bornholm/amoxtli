...
// speed-up that turns a 35-minute evaluation ingestion into a 6-minute one. Most
// of that gap is not the backend but the batching: Ollama serves
// /v1/embeddings one request at a time, while this client packs as many inputs
// into a single llama.cpp batch as the physical batch size allows. See
// docs/embeddings-backend-eval.md for the measurements.
//
